...
对策：
1. **搜文件内容一律用 Grep 工具**，不要用 bash `grep`，也不要 `git show/log/diff ... | grep` 这种管道。
...
```
- 非 0 → `git -C d:/work/linze-journal checkout learning/interview-tools/stealth.html` 全撤销，返回 `结构失败已撤销`，**绝不提交**。
- 通过 → bump `sw.js` CACHE_NAME → `git -C d:/work/linze-journal add learning/interview-tools/stealth.html sw.js && git -C d:/work/linze-journal commit -m "loop-review: 通过X·退回Y·放弃Z — vN" && git -C d:/work/linze-journal push origin main`。push 失败 → 返回 `push失败`，不无限重试。
