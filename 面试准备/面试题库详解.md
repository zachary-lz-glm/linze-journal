...
前端渲染引擎是整个架构最精巧的部分，核心是**组件注册表 + 动态映射 + 两级联动 HOC**：

1. **组件注册表**（`app.tsx`）：
   ```typescript
   export const RegisteredComponents = new Register();
   const cmps = [
     { cmpName: "TargetCrowdCmp", cmp: (props) => <FormTargetCrowd {...props} /> },
     { cmpName: "RewardCondition", cmp: (props) => <RewardCondition {...props} /> },
     { cmpName: "TimeRangeCmp", cmp: (props) => <FormTimeRangeCmp {...props} /> },
     // ... 35+ 注册组件
   ];
   RegisteredComponents.addMultipleCmps(cmps);
   ```
   Schema 的 `d_component` 字段 → 字符串名 → 注册表查表 → 对应 React 组件，零硬编码。

2. **Schema 迭代渲染**（`StepForm`）：
   ```typescript
   schemaList?.map((item, index) => {
     return <FieldWrapper
       key={item.name}
       {...item}                    // 展开整个 Schema 对象
       form={createForm}            // Form 实例
       registeredComponents={RegisteredComponents}
       onPreviewChange={onPreviewChange}
     />
   })
   ```
   BFF 返回的 schemaList 是字段数组，前端逐个渲染。每个字段的 `d_component` 决定用哪个组件。

3. **两级联动 HOC**（`ConditionMethodCmp.tsx`）：
   - **`DynamicComponent`**（同级联动）：监听当前表单内其他字段值，决定显隐
     ```typescript
     const value = formInstance?.getFieldValue(formatName(groupID, dependPath));
     if (dependValue.includes(value)) {
       return <WrappedComponent {...props} />  // 匹配则渲染
     }
     formInstance?.setFieldValue(..., undefined);  // 隐藏时清空值
     return null;
     ```
   - **`DynamicComponentWithBFF`**（跨级联动）：基于 BFF 返回的联动数据，支持跨步骤依赖
     ```typescript
     const isArrayMatch = Array.isArray(bffDependValue) && dependValue?.some(...);
     const isExcluded = dependExclude && ...
     const isMatched = (isArrayMatch || isSingleMatch) && !isExcluded;
     ```
   - 隐藏时自动清空字段值，显示时恢复，用 `useRef` 跟踪防止重复触发。

4. **Schema 类型定义**（`types.ts`）：
   ```typescript
   export interface IFormFieldProps {
     d_component: string;     // → 组件注册表映射
     d_actions: any[];        // → 联动动作定义
     visible?: boolean;       // → 显隐控制
     disabled?: boolean;      // → 禁用控制
     contentSchema?: any;     // → 一层嵌套 Schema
     preview?: string;        // → 'basic' | 'mobile' 实时预览
     // ... name, label, value, rule, options 等
   }
   ```

5. **状态管理**：Redux（`saveFormData`）+ Ant Design Form 实例 + 自定义 Listener 事件系统
   - Redux 存全局表单数据，支持跨步骤读取
   - Form 实例做字段级操作（getFieldValue / setFieldValue）
   - Listener 事件系统处理字段变更通知和 C 端实时预览

**面试说法：**

> 我们的 Schema 驱动架构分两层。BFF 层根据活动类型生成 Schema——字段结构、组件类型、联动规则。前端渲染引擎拿到 Schema 后，通过组件注册表做动态映射：Schema 的 d_component 字段是字符串名，注册表里 35+ 组件按名字匹配，零硬编码。联动用两级 HOC：DynamicComponent 处理同级联动，比如选了"推送"才显示推送模板字段；DynamicComponentWithBFF 处理跨级联动，基于 BFF 返回的联动数据。隐藏时自动清空值，用 ref 跟踪防止重复触发。状态管理是 Redux + Form 实例 + 自定义 Listener 事件系统三层。这样新增活动类型前端完全不用写代码，只要 BFF 配好 Schema。

**加分点：** 能画出完整的 BFF Schema → 前端渲染引擎 → 两级联动 HOC 的数据流 → 体现全链路理解
