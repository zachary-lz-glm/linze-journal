...
> Agent / 技术项目卡（Manager、Code 等）是对 §1.1 口语化的**例外**：结构化参考卡风格、全覆盖源文档、概念层不堆配置数——**详见 §6**。
...
- **proj**：Schema 项目 → **Manager 智能体**(14卡，2026-07-17 重构 + 2026-07-20 补认知层 3 卡) → **Code 智能体**(19卡，2026-07-17 重构 + 2026-07-20 补认知层 2 卡) → AI 总览/架构/质量/端到端/工具 → 产品思维
...
6. **并发改同一文件会串 commit**（2026-07-17 踩到）：两个 agent 同时改 stealth.html，一个 `git add stealth.html`（整文件）会把另一个工作区里还没提交的改动**一起带进自己的 commit**——改动挂在错的 commit 消息下、或你以为没推其实已经在 origin 上了。并发时：各自只改自己区域、提交前先 `git pull` / `git status` 确认工作区干净、改完立刻提交别拖。
7. **sw.js bump 要看"已部署版本"，不是"本地最新 commit 号"**：kb-vN 要不要 +1，取决于上一个**已经推到 origin / 部署**的版本。两个 agent 各自 bump 容易撞号——后推的要先看 origin 上当前的 `CACHE_NAME` 再 +1；否则推了同版本、sw.js 字节不变，Service Worker 不触发更新，访客一直拿旧缓存。
...
- [ ] 标题没有英文项目名前缀？
- [ ] 语气档位对了？轻卡口语；Agent/技术项目卡用结构化参考卡风格（`<strong>段首。</strong>` + ①② 内联，对标「从 PRD 到上线」）？（见 §1.1 例外 + §6）
- [ ] 速查表腔（必背 / 杀手锏 / 被问 X 必答 / 金句钩子）清掉了？①② 只内联枚举、不堆成 ①②③④⑤、不当标签。（见 §6.1）
- [ ] 覆盖了源文档（学习指南 + README）的知识地图和面试常问点？自测清单每条有落点卡？（见 §6.3）
- [ ] 概念层没堆魔法数字？只有机制 / 亮点卡带关键数？（见 §6.4）
- [ ] 每个数字都核对过源码 / 能答出"怎么测的"？
- [ ] 诚实边界（没实现的、已知债）有没有主动说？
- [ ] `data-kw` 覆盖了这道题的各种问法？
- [ ] div 开/闭标签平衡？`<script>` 完整？
- [ ] 新子组在 `subOrder` 里有排序值？`getGroup` 能正确路由？
- [ ] 改了 stealth.html 就 bump 了 sw.js？bump 看的是 origin 已部署版本 +1？（见 §4.7）
- [ ] 并发改时，提交前 `git pull` / `git status` 确认过工作区、没串进别人改动？（见 §4.6）
...
## 7. 实现细节深挖卡（2026-07-23 · D 子项目实战，第四种卡型）

> 第四种卡型，补齐 stealth 的卡生态：**主陈述卡**（五步，开场讲为什么决策）→ **机制弹药卡**（单点机制讲我的取舍）→ **实现深挖卡**（讲数据结构/算法/代码怎么造的）。被面试官追问"这个具体怎么设计的 / 给我讲讲实现细节 / 数据结构长什么样"时翻这张。参考实现：D 子项目 4 张（commit `6180732`, kb-v328），spec `docs/superpowers/specs/2026-07-22-stealth-deepdive-cards-design.md`。

### 7.1 形态：Form A 四要素（结构化实现卡，对标 §6 参考卡风格）

4–5 段，每段 `<strong>领起标签。</strong>` 开头（≥4 段），declarative 不碎嘴。**必含四要素**——这是它与"纯技术流水账"（storytelling memory 点名的头号反模式）的分界：

1. **数据结构 / 形态**——长什么样（JSON schema / 文件结构 / graph 节点边 / DSL 语法）。
2. **核心算法 / 流程**——怎么跑（余弦 top-k / 蒸馏流程 / probe→plan / 双求值）。
3. **为什么自研、拒绝了什么 alternative + 代价**（CLEAR 的 L + 三段推理链）。
4. **踩坑 + 诚实边界**（eval / 局限）——**第 4 项不可省**。

可选第 5 段：**代码引用**（`file.ts:行号`）——深挖卡讲实现，允许带文件名/行号，但**只进 body、绝不进 title**。

### 7.2 源码核对策略（关键，D 验证有效）

- **有源码的项目**（agent 项目）：派 agent **先读源码产「事实清单 + ✅⚠️❌ 可说清单」，再写卡**。D 实测：Manager 39 节点（文档写 38、代码实数 39，`fix` 节点后加）、Code 两套自研向量库 schema、PRD 6 yaml——全是读源码核对出来的，不是编的。
- **无源码的项目**（前端）：靠 spec 素材 + 现有卡 + 记忆，**inferred 的实现标签（函数名/API 名）一律软化为概念**（D 把 safeEval/FetchApi/listener.on 软化成"求值/回 BFF 取配置"，因前端无源码无法确认具体名）。
- 源码路径（mac）：`/Users/didi/work/agent-main/Manager_Agent/`、`/Users/didi/work/agent-main/code_assistent_Agent/`、`/Users/didi/work/prd-tools/`。

### 7.3 诚实处理模式（D 总结，复用）

- **文档 vs 代码数字不一致**：以代码为准，面试说模糊数（"接近 40"最稳，别说死 38 或 39）。
- **源码里没硬编的值绝不报**（D：embedding 向量维度源码没硬编，就明说"模型决定、不报数字"，不临时编）。
- **eval 债主动认**（"没建 labeled query set 测 recall@K / nDCG，是已知评测债"）——acknowledge gaps still get hired。
- 雷区词（Chroma / git 钩子 / Claude / GPT / SDD 平台 / 6 步闭环）**可在"不用/拒绝/对比"否定语境出现**，不能当自述。check 脚本对这些用 warn + 人工确认语境，不用 naive err（会误报）。

### 7.4 routing：插主卡正后方，不是 body-swap

深挖卡是**新增卡**（不是改现有 body），assemble 脚本按**主陈述卡 title 锚点 → depth 数到主卡闭合 `</div>` → 在其后插入**（参考 `assemble_d.py`）。挂现有 `data-g`、`subOrder` 不动（组内按 DOM 序展示，插对位置即可）。每张插在该项目主陈述卡**正后方**——面试翻主卡能顺带翻到实现。

### 7.5 校验脚本（check_d.py 模式，复用）

`check_d.py <片段> --project front|mgr|code|prd`：四要素词命中（数据结构/算法/拒绝/诚实 各≥1，warn 不阻塞）+ strong≥4 + 禁碎嘴 + ①②③④⑤不堆叠 + div 平衡 + 完整 card 片段 + "6 步闭环"硬 err + **per-project 雷区 warn**（Chroma/git钩子/Claude/SDD/20+人，warn 靠人工确认语境）。
