...
#set text(font: ("PingFang SC", "Helvetica Neue"), size: 10pt, lang: "zh", fill: rgb("#222222"))
...
#hl[AI 工程化方向独立落地者] —— 独立设计并落地 SDD（Spec-Driven Development）体系：6 文件 SSOT 知识库 + 11 步 PRD→Spec 蒸馏工作流 + 8 种证据链 + 三层质量门控，清晰区分 Vibe Coding 与标准化 AI 研发的边界。

#set list(marker: text(fill: accent)[•], indent: 2pt, body-indent: 5pt)
...
#hl[AI 工程化（差异化优势）]：SDD / Spec-Driven · Claude Code Skill 工作流 · Prompt Engineering · Context Engineering · Harness Engineering · RAG · MCP 协议 · Function Calling · LangGraph · Agent Loop · Claude API
...
#text(weight: "bold")[行动]：
- #hl[双 Skill 知识底座]：知识库构建 6 模式 + Spec 蒸馏 11 步工作流，反馈回流闭环让 Spec 越用越准。
...
#text(size: 8.5pt, fill: rgb("#666666"))[前端 + BFF 架构设计（与导师 5-5 主导）· React · TypeScript · Node.js Serverless · Lerna + Nx]
...
- #hl[Rollup 双模打包]（ESM + CJS + 类型声明）+ peerDependencies 隔离 + Changesets 语义化发版；接入支持组件 / 函数调用 / 微前端三形态。

#text(weight: "bold")[结果]：覆盖 #hl[8 个业务项目]，累计接入 #hl[400+ 种权益 SKU]（底层组件 30-40 个），新权益接入从 #hl[2 天缩短到 2 小时]，零 breaking change 运行 #hl[18 个月+]。

== 加分项

- #hl[AI Native App 0-1 落地]：prd-tools 从底层设计 AI 优先工作流范式（非给已有产品加 AI），完整闭环：知识库 → PRD 蒸馏 → Spec/Plan → 人工 Review → 反馈回流。
- #hl[Claude Code 插件开发]：独立开发 2 个 Skill 插件（build-reference + prd-distill），零依赖设计（不依赖 Node / LLM Key），团队拿到即用。
- #hl[团队技术影响力]：主导《中台开发规范》《BFF 接口规范》《CI/CD 流水线规范》落地及多场业务技术分享；B 端 20+ 人团队 AI 工作流推广者。
