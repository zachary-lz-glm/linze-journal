# 面试准备 W1 周三日报 — 2026-04-29

## 一、今日核心事件

**ibt-benefit-sdk 权益组件库深度复盘**。从源码逐文件精读，梳理出 6 个面试讲点，纠正 1 个认知偏差，发现 1 个重大架构洞察。
...
**一个纠正**：
1. 面试文档 A3 说"Register 注册模式，动态注册扩展点" — 实际是构造时静态注册 24 个组件 + `add()`/`addMultipleCmps()` 运行时扩展。不是纯动态注册，而是"内置基础 + 开放扩展"。
...
面试官可能会问："benefit SDK 和 营销平台 的组件库有什么不同？"
...
- [ ] prd2code-gen 复盘：3-Skill AI 工具链的 5 个面试讲点
